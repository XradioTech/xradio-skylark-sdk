...
#include "net/libwebsockets/config/lws_config.h"
#include "net/libwebsockets/config/lws_config_private.h"
...
#include <inttypes.h>
 ...
 #else
  #if !defined(LWS_WITH_ESP8266) && !defined(LWS_WITH_ESP32) && !defined(LWS_WITH_XRADIO)
 ...
 #define strerror(x) ""
   ...
   #define OPENSSL_NO_TLSEXT
    ...
    #include "tls/mbedtls/wrapper/include/openssl/ssl.h" /* wrapper !!!! */
	 ...
	 #include "net/libwebsockets/ssl.h"
	#else
     #include <mbedtls/ssl.h>
     #include <mbedtls/x509_crt.h>
     #include <mbedtls/x509_csr.h>
...
#define MAX_WEBSOCKET_04_KEY_LEN 128
 ...
 #ifdef LWS_WITH_XRADIO
  #define SYSTEM_RANDOM_FILEPATH "null"
 #else
...
struct lws_vhost {
	...
	char *key_path;
#if !defined(LWS_WITH_ESP8266) && !defined(LWS_WITH_ESP32) && !defined(OPTEE_TA) && !defined(WIN32) && !defined(LWS_WITH_XRADIO)
	int bind_iface;
#endif
...
#define lws_free(P)	lws_realloc(P, 1, "lws_free")
#define lws_free_set_NULL(P)	do { lws_realloc(P, 1, "free"); (P) = NULL; } while(0)
#endif
