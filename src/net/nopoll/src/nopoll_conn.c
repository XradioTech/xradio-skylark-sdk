...
nopoll_bool                 nopoll_conn_set_sock_block         (NOPOLL_SOCKET socket,
								nopoll_bool   enable)
{
#if defined(NOPOLL_OS_UNIX) || (defined(NOPOLL_OS_FREERTOS) && defined(NOPOLL_LWIP))
	int  flags;
#endif
				       ...
				       const char      * origin)
{
	...
	if (enable_tls) {
#if defined(NOPOLL_MBEDTLS)
		conn->fd_ctx		= nopoll_new(mbedtls_net_context, 1);
		conn->ssl			= nopoll_new(mbedtls_ssl_context, 1);
		conn->ssl_conf		= nopoll_new(mbedtls_ssl_config, 1);
		conn->ssl_cert		= nopoll_new(mbedtls_x509_crt, 1);
		conn->ssl_ca_cert	= nopoll_new(mbedtls_x509_crt, 1);
		conn->ssl_pkey		= nopoll_new(mbedtls_pk_context, 1);
		conn->ssl_entropy	= nopoll_new(mbedtls_entropy_context, 1);
		conn->ssl_ctr_drbg	= nopoll_new(mbedtls_ctr_drbg_context, 1);
...
#if defined(NOPOLL_MBEDTLS)
	if (conn->fd_ctx) {
		nopoll_free(conn->fd_ctx);
	}
	if (conn->ssl) {
		mbedtls_ssl_free(conn->ssl);
		nopoll_free(conn->ssl);
	}
	if (conn->ssl_conf) {
		mbedtls_ssl_config_free(conn->ssl_conf);
		nopoll_free(conn->ssl_conf);
	}
	if (conn->ssl_cert) {
		mbedtls_x509_crt_free(conn->ssl_cert);
		nopoll_free(conn->ssl_cert);
	}
	if (conn->ssl_ca_cert) {
		mbedtls_x509_crt_free(conn->ssl_ca_cert);
		nopoll_free(conn->ssl_ca_cert);
	}
	if (conn->ssl_pkey) {
		mbedtls_pk_free(conn->ssl_pkey);
		nopoll_free(conn->ssl_pkey);
	}
	if (conn->ssl_entropy) {
		mbedtls_entropy_free(conn->ssl_entropy);
		nopoll_free(conn->ssl_entropy);
	}
	if (conn->ssl_ctr_drbg) {
		mbedtls_ctr_drbg_free(conn->ssl_ctr_drbg);
		nopoll_free(conn->ssl_ctr_drbg);
	}
#else
	if (conn->ssl)
		SSL_free (conn->ssl);
...
#elif defined(NOPOLL_OS_FREERTOS)
		OS_SetErrno(NOPOLL_EWOULDBLOCK);
#endif
		return NULL;
...
#elif defined(NOPOLL_OS_FREERTOS)
		nopoll_freertos_gettimeofday(&start, NULL);
#else
...
#elif defined(NOPOLL_OS_FREERTOS)
		nopoll_freertos_gettimeofday(&stop, NULL);
#else
...
#elif defined(NOPOLL_OS_FREERTOS)
		mask_value = (unsigned int)OS_Rand32();
#else
