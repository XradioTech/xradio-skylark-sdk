...
_shttpd_stop_stream(struct stream *stream)
{
	...
	if (stream->io_class != NULL && stream->io_class->io_close != NULL)
		stream->io_class->io_close(stream);
...
decide_what_to_do(struct conn *c)
{
	...
	if (c->method == METHOD_PUT) {
		...
		} else if ((rc = _shttpd_put_dir(path)) == 0) {/*create dir or file*/
...
read_stream(struct stream *stream)
{
	...
	n = stream->io_class->io_read(stream, io_space(&stream->io), len);
	if (n > 0)
...
write_stream(struct stream *from, struct stream *to)
{
	...
	n = to->io_class->io_write(to, io_data(&from->io), len);
...
connection_desctructor(struct llhead *lp)
{
	...
	if (!do_close && c->loc.content_len > 0) {
		...
		DBG(("Destroy connection.\n"));
		if (c->rem.io_class != NULL)
			c->rem.io_class->io_close(&c->rem);
...
#else
		_shttpd_elog(E_FATAL, NULL, "only support one work.\n");
#endif
	} else {
...
#if defined(SHTTPD_SINGLE_CONNECTION)
...
shttpd_socketpair(int sp[2])
{
	...
	if ((sock = socket(AF_INET, SOCK_STREAM, 0)) != -1 &&
	  ...
	  //  (sp[0] = socket(AF_INET, SOCK_STREAM, 6)) != -1 &&
	   (sp[0] = socket(AF_INET, SOCK_STREAM, 0)) != -1 &&
...
#if defined(SHTTPD_FS)  || !defined (SHTTPD_LOG_ALT)
static int
...
process_command_line_arguments(struct shttpd_ctx *ctx, char *argv[])
{
	...
	FILE			*fp;
	size_t			i, line_no = 0;
	_shttpd_elog(E_LOG, NULL, "parse cmd line......\n");
