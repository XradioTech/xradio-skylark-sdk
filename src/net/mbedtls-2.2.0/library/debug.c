...
void* tls_debug_zalloc(unsigned int size)
{
        void *p;
                      ...
                      const char *text, const mbedtls_mpi *X )
{
    ...
    mbedtls_snprintf( str + idx, DEBUG_BUF_SIZE/*sizeof( str )*/ - idx, "value of '%s' (%d bits) is:\n",
    ...
    for( i = n + 1, j = 0; i > 0; i-- )
    {
        ...
        for( k = sizeof( mbedtls_mpi_uint ) - 1; k >= 0; k-- )
        {
            ...
            if( j % 16 == 0 )
            {
                if( j > 0 )
                {
                    mbedtls_snprintf( str + idx, DEBUG_BUF_SIZE/*sizeof( str )*/ - idx, "\n" );
                    debug_send_line( ssl, level, file, line, str );
                    idx = 0;
                }
            }

            idx += mbedtls_snprintf( str + idx, DEBUG_BUF_SIZE/*sizeof( str )*/ - idx, " %02x", (unsigned int)
                             ( X->p[i - 1] >> ( k << 3 ) ) & 0xFF );
    ...
    if( zeros == 1 )
        idx += mbedtls_snprintf( str + idx, DEBUG_BUF_SIZE/*sizeof( str )*/ - idx, " 00" );
                      ...
                      const char *text, const mbedtls_x509_crt *crt )
{
    ...
    while( crt != NULL )
    {
    	...
    	char buf[1024];
#else
    	char *buf = tls_debug_zalloc(1024);
    	if (buf == NULL)
    		goto exit;
#endif


        mbedtls_snprintf( str, DEBUG_BUF_SIZE/*sizeof( str )*/, "%s #%d:\n", text, ++i );
        debug_send_line( ssl, level, file, line, str );

        mbedtls_x509_crt_info( buf, 1024/*sizeof( buf )*/ - 1, "", crt );
